...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response
{
    ...
    {
        model = [[IAPProductModel alloc] initWithName:@"AVAILABLE PRODUCTS" elements:response.products];
        ...
        // EHLog(@"id: %@, price: %@", p.productIdentifier, p.price);
    }
    ...
    {
        model = [[IAPProductModel alloc] initWithName:@"INVALID PRODUCT IDS" elements:response.invalidProductIdentifiers];
        [self.productRequestResponse addObject:model];
        EHLog(@"invalid identifiers-->%@", response.invalidProductIdentifiers[0]);
    }
...
- (void)request:(SKRequest *)request didFailWithError:(NSError *)error
{
    ...
    EHLog(@"Product Request Status: %@",error.localizedDescription);
}
